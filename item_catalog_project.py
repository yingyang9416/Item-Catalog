...
def gconnect():
 ...
 # This no longer used:  login_session['credentials'] = credentials
    ...
    flash("you are now logged in as test %s" % login_session['username'])
    print "done!"
    print login_session
    return output
...
def showBooks(library_id):
    library = session.query(Library).filter_by(id = library_id).one()
    books = session.query(Book).filter_by(library_id = library_id).all()
    return render_template('books.html', books = books, library = library)
...
def newBook(library_id):
  library = session.query(Library).filter_by(id = library_id).one()
  if request.method == 'POST':
      newBook = Book(name = request.form['name'], description = request.form['description'], author = request.form['author'], published_year = request.form['published_year'], library_id = library_id)
      session.add(newBook)
      session.commit()
...
#Edit a book
@app.route('/library/<int:library_id>/books/<int:book_id>/edit', methods=['GET','POST'])
def editBook(library_id, book_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editBooks.html', library_id = library_id, book_id=book_id, book=editedBook)

        


#Delete book
@app.route('/library/<int:library_id>/books/<int:book_id>/delete', methods = ['GET','POST'])
def deleteBook(library_id,book_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteBook.html', book = bookToDelete,library_id = library_id)
